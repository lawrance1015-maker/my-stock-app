...
if page == "持倉管理":
    ...
    if st.session_state.portfolio:
        st.write("---")
        # 標題行
        h1, h2, h3, h4, h5 = st.columns([2.5, 2, 2, 1, 1])
        h1.write("**股票名稱 / 代號**")
        h2.write("**報價 / 成本**")
        h3.write("**盈虧與派息**")
        h4.write("**修改**")
        ...
        for index, item in enumerate(st.session_state.portfolio):
            try:
                ...
                annual_div = div_rate * item['shares']
                total_est_dividend += annual_div
                ...
                with c3:
                    ...
                    st.markdown(f":{color}[**${profit:,.2f}** ({profit_pct:+.2f}%)]")
                    if div_rate > 0:
                        st.caption(f"🎁 預計年息: ${annual_div:,.1f} ({div_yield:.1f}%)")
                    else:
                        st.caption("🚫 無派息數據")
        ...
        # 顯示底部匯總卡片
        if total_est_dividend > 0:
            st.info(f"💰 根據目前持倉，預計全體年度總股息收入約為: **${total_est_dividend:,.2f}**")
